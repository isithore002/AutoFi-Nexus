...
const GlassCard = ({ children, className = '', delay = 0 }: { children: React.ReactNode; className?: string; delay?: number }) => (
  <motion.div
    ...
    className={`rounded-2xl bg-gradient-to-br from-gray-800/50 to-gray-900/50 border border-gray-700/30 shadow-[0_8px_32px_rgba(0,0,0,0.4)] p-6 ${className}`}
  >
    {children}
  </motion.div>
);
...
const GradientButton = ({ children, onClick, className = '', variant = 'primary' }: { children: React.ReactNode; onClick?: () => void; className?: string; variant?: 'primary' | 'outline' }) => {
  const variants = {
    ...
    outline: 'border-2 border-white/30 bg-white/5'
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white overflow-hidden relative">
      ...
      <motion.nav 
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        className="bg-gray-900/80 backdrop-blur-sm border-b border-gray-700/50 px-8 py-4 sticky top-0 z-50"
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          {/* Navigation Tabs */}
          <div className="hidden md:flex gap-1 bg-gray-800/50 p-1 rounded-xl border border-gray-700/30">
          ...
          <div className="flex items-center gap-4">
            <motion.button
              ...
              className="w-10 h-10 rounded-xl bg-gray-800/50 border border-gray-700/30 flex items-center justify-center hover:bg-gray-700/50 transition-all"
            >
              {isDark ? <Moon size={20} /> : <Sun size={20} />}
            </motion.button>
            <GradientButton onClick={() => setConnected(!connected)}>
              <div className="flex items-center gap-2">
                <Wallet size={18} />
                {connected ? '0x742d...5f3A' : 'Connect Wallet'}
              </div>
            </GradientButton>
          </div>
        </div>
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap mt-2">
                  {portfolioData.chains.map(chain => (
                    <span key={chain} className="px-2 py-1 bg-gray-700/50 border border-gray-600/30 rounded text-xs">
                      {chain}
                    </span>
                  ))}
                </div>
          ...
          <div className="space-y-4">
            {strategies.map((strategy, idx) => (
              <motion.div
                ...
                className="bg-gray-800/30 rounded-xl p-4 border border-gray-700/20 hover:bg-gray-700/30 transition-all group"
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <motion.button
                      whileHover={{ scale: 1.1 }}
                      className="w-10 h-10 rounded-lg bg-gray-700/50 border border-gray-600/30 flex items-center justify-center group-hover:bg-gray-600/50 transition-all"
                    >
            ...
            <div className="space-y-3">
              {chainActivity.map((item, idx) => (
                <motion.div
                  ...
                  className="flex items-center justify-between p-3 bg-gray-800/30 rounded-lg border border-gray-700/20 hover:bg-gray-700/40 transition-all"
                >
                  <div className="flex items-center gap-3">
              ...
              <AnimatePresence>
                {activities.map((activity, idx) => (
                  <motion.div
                    ...
                    className="flex items-center justify-between p-3 bg-gray-800/30 rounded-lg border border-gray-700/20 hover:bg-gray-700/40 transition-all group"
                  >
                    <div className="flex items-center gap-3">
